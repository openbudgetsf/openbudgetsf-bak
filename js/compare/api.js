...
export function fetchBreakdownData (years, yearTypes, type, dimension) {
  ...
  const urls = years.map((year) => {
    return config.compare.apiBase + typePaths[type] + dimensionPaths[dimension] + '/' +
           year + config.compare.breakdownUrlSuffix;
  });
...
export function fetchTotals () {
  return axios.get(config.compare.apiBase + typePaths.spending + config.compare.totalsUrlSuffix)
  .then(response => {
